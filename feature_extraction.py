...
enc = keras.Model(model.get_layer("sequential").input,
                  model.get_layer("sequential").output)


def decode_img(img_path: tf.Tensor) -> tf.Tensor:
...
MAX_SEQ_LEN = 160
data: pd.DataFrame = pd.read_csv("data/cropped/labels.csv",
                                 sep=":  ", names=["ids", "label"])
...
def process_image(video_id: tf.Tensor, label: tf.Tensor) -> Tuple[Dict[str, tf.Tensor], tf.Tensor]:
    this_features = enc(decode_img(video_id))
    paddings = [[MAX_SEQ_LEN-1-tf.shape(this_features)[0], 0], [0, 0]]
